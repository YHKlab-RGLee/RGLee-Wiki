...
예를 들어 소스 쪽의 $V(x)$가 0에 가깝고 드레인 쪽으로 갈수록 증가하면, 같은 $V_{GS}$에서도 드레인 쪽의 $|Q_\mathrm{inv}|$가 더 작다. 이 공간적 전하 감소가 다음 절의 pinch-off와 saturation을 이해하는 출발점이다.[1,2]
