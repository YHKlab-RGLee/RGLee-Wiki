...
다음 순서는 개별 셀의 판독 원리가 아니라 주소가 선택되고 데이터가 칩 밖으로 전달되는 **기능 블록의 의존 관계**이다.[3–6]
...
일반적인 쓰기는 입력 데이터가 반대 방향으로 이동한다.[3,4]
...
메모리 기술을 비교할 때에는 “빠르다” 또는 “고밀도이다”라는 단일 표현보다 측정 경계와 단위를 명시한 지표가 필요하다. 특히 read와 write가 비대칭이고 내부 병렬성이 큰 메모리에서는 cell, array, chip과 system 수치를 섞지 않아야 한다.[1,7–9]
