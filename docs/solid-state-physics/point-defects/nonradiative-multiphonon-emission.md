...
## 1. Capture Rate와 Electron–Phonon Coupling

### (1) Macroscopic Capture Coefficient
...
Capture coefficient는 단순한 결함의 기하학적 면적이 아니다. Electron–phonon coupling, 초기·최종 vibrational state의 overlap, 전하 상태의 long-range Coulomb interaction과 온도를 모두 포함한다.[1,2]
...
초기 vibrational state $m$의 thermal occupation을 $w_m(T)$라 하면 한 결함에 대한 transition rate는
...
### (2) One-Dimensional Reduction과 Numerical Sum
...
실제 계산에서 delta 함수를 유한 폭의 Gaussian 등으로 바꾸면 energy-conservation error와 capture coefficient의 폭 의존성이 생긴다. Vibrational level의 이산 합을 충분히 수렴시키고, broadening을 바꿨을 때 결과가 안정한지 확인해야 한다.[2,4]
...
1. NMP capture는 전자 상태 변화와 서로 다른 전하 상태의 nuclear vibrational-state overlap이 결합된 과정이다.
2. Mass-weighted $\Delta Q$와 두 energy surface는 lattice relaxation을 정량화하며, Huang–Rhys factor는 displaced harmonic oscillator의 결합 척도이다.
3. Static-coupling Fermi golden rule에서 $W_{if}$와 진동 행렬 원소가 미시적 전이율을 정한다.
4. 주기 supercell transition rate는 계산 부피와 Coulomb factor를 적용해 capture coefficient로 변환해야 한다.
